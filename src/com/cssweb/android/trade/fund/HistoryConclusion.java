 ...
 *
 */
public class HistoryConclusion  extends TradeQueryBase  {
	private String startdate;
	private String enddate;
	private String DEBUG_TAG ="HistoryConclusion";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		colsName = getResources().getStringArray(R.array.historyconclusion_colsname);
		colsIndex = getResources().getStringArray(R.array.historyconclusion_colsindex);
			...
			r = new Runnable(){
				public void run() {
					if(btnTag == -1 || btnTag == 3){
						if(type == 1){
							try {
								quoteData = FundService.getHistoryTrade(startdate, enddate);
	...
	protected void handlerData() {
		if(quoteData == null){
			...
			if (type == 1 || btnTag == 3) {// 进去页面请求 或 刷新
				Toast.makeText(HistoryConclusion.this, "读取数据失败！请刷新或者重新设置网络。。", Toast.LENGTH_LONG).show();
	...
	protected void displayDetails() {
		...
		forwardDetails(HistoryConclusion.this);
	}
